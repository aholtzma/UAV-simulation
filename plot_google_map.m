...
%
% USAGE:
% h = get_google_map(Property, Value,...)
% Plots the map on the given axes. Used also if no output is specified
%
% Or:
% [lonVec latVec imag] = get_google_map(Property, Value,...)
...
%    MapType          -   ('mobile')  Type of map to return. Any of [roadmap, mobile,
%                                     satellite, terrain, hybrid, mapmaker-roadmap,
%                                     mapmaker-hybrid) See the Google Maps API for
%                                     more information. mobile is usually same as
%                                    roadmap, but includes more details in some
%                                    areas.
...
%    h = plot_google_map('maptype','roadmap')
%
% References:
...
if nargout <= 1 % only if in plotting mode
    ...
    for childIdx = 1:length(curChildren)
        if isequal(get(curChildren(childIdx),'tag'),'gmap')
            delete(curChildren(childIdx));
        end
    end
end
...
sizeStr = ['&size=' num2str(width) 'x' num2str(height)];
...
sensor = '&sensor=false';
url = [preamble location zoomStr sizeStr maptypeStr format markers sensor];
...
% Drawback is longer processing time - INTERP2 is a bit heavy for 640X640.
...
if nargout <= 1 % plot map
    ...
    set(gca,'children',children([ 2:end 1] ))
